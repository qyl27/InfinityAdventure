...
public class TrapperEntity extends Monster implements IAnimatable {
    ...
    private <E extends IAnimatable> PlayState animationPredicate(AnimationEvent<E> event) {
        if (event.isMoving()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("trapper.move", ILoopType.EDefaultLoopTypes.LOOP));
            return PlayState.CONTINUE;
        }

        // Todo: qyl27: attack animations.
        event.getController().setAnimation(new AnimationBuilder().addAnimation("trapper.idle", ILoopType.EDefaultLoopTypes.LOOP));
        return PlayState.CONTINUE;
    }
